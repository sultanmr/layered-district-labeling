...
class SbahnLlmSegmenter(LLMSegmentationStrategy):
    def __init__(self):
        super().__init__()
    ...
    def segment(self, features: pd.DataFrame):
        ...
        for _, row in features.iterrows():
            ...
            summary = {
                'sbahn_per_capita': row['sbahn_per_capita'],
                'num_sbahn': row['num_sbahn']
            }
    ...
    def _create_prompt(self, district_summary: dict) -> str:
        return f"""
        Analyze these sbahn statistics for a district:

        District Profile:
        - Sbahn per capita: {district_summary['sbahn_per_capita']:.4f}
        - Number of sbahn: {district_summary['num_sbahn']}
