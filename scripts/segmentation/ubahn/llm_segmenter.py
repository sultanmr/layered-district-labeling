...
class UbahnLlmSegmenter(LLMSegmentationStrategy):
    def __init__(self):
        super().__init__()
    ...
    def segment(self, features: pd.DataFrame):
        ...
        for _, row in features.iterrows():
            ...
            summary = {
                'ubahn_per_capita': row['ubahn_per_capita'],
                'num_ubahn': row['num_ubahn']
            }
    ...
    def _create_prompt(self, district_summary: dict) -> str:
        return f"""
        Analyze these ubahn statistics for a district:

        District Profile:
        - Ubahn per capita: {district_summary['ubahn_per_capita']:.4f}
        - Number of ubahn: {district_summary['num_ubahn']}
