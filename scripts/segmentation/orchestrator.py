...
class SegmentationOrchestrator:
    ...
    def _store_results(self, results: pd.DataFrame):
        ...
        results.to_sql('labels', self.engine,
    ...
    def network_graph(self, data: pd.DataFrame, title: str, save_path: str):
        ...
        for _, row in data.iterrows():
            ...
            G.add_node(row['hashtags'], 
                       type='tag',
                       size=600)
            G.add_edge(row['district'], row['hashtags'])
        ...
        plt.title("District-Tag Relationship Network", pad=20)
        plt.axis('off')
        plt.tight_layout()
    ...
    def visualize_results(self, results, image_path):
        ...
        try:
            ...
            neighborhood_tags_heatmap_path = "heatmap.png"
            neighborhood_tag_network_path = "network.png"
            csv_file_path = "labels.csv"
            
            if image_path is not None:
                if not os.path.exists(image_path):
                    os.mkdir(image_path)
                neighborhood_tags_heatmap_path = os.path.join(image_path, neighborhood_tags_heatmap_path)
                neighborhood_tag_network_path = os.path.join(image_path, neighborhood_tag_network_path)
            ...
            for table_name in results['table_name'].unique():
                ...
                tags_expanded = df.explode('hashtags')
                heatmap_data = pd.crosstab(
                    tags_expanded['district'],
                    tags_expanded['hashtags']
                )
                self.heatmap (heatmap_data, "District Segment Tags", neighborhood_tags_heatmap_path)

                try:
                    self.network_graph(tags_expanded, "District-Tag Network", neighborhood_tag_network_path)
                except ImportError:
                    logging.warning("Network visualization skipped (networkx not available)")
    ...
    def add(self, id: str, segment_types = None):
        ...
        Args:
            segment_type: The segment type name in snake_case format (e.g. 'parks')
